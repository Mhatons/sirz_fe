    ...
    return (
        <div className="  bg-background_light dark:bg-background_dark dark:text-white pt-12 pb-8">
            <header className=" text-center pb-4 sm:w-[60%] w-[90%] m-auto">
                ...
                <div className='text-lg'>Our expert designers bring a wealth of experience from transforming spaces for a diverse range of residential, commercial, and corporate clients.</div>
                <div>Check our core values below</div>
            </header>
            <ul className=" p-4 text-[12px] font-comfortaa ">
                <Slider {...settings}>
                    {
                        coreValues.map((value, index) => (
                            <div className=' px-2'>
                                <li key={index} className=" shadow-inner rounded-lg px-4 py-2 h-20 border">
                                    <header className=" text-primary_dark text-sm font-semibold  pb-1 rounded-sm">{value.title}</header>
