...
export default function AnimatedCard(props: Idata) {
    ...
    return (
        ...
        >
            ...
            <div className='bg-[#F9FCFF] dark:bg-background_dark dark:text-background_light animatedcardtext absolute bottom-0 p-2 rounded-3xl rounded-br-none sm:w-[90%] w-[95%]'>
                <p className=" text-zinc-400">Step {index}</p>
                <h4 className=' py-0 text-secondary text-[16px] font-semibold'>{title}</h4>
                <div className='animatedcardtext_desc text-[13px] text-justify text-zinc-200'>{desc}</div>
            </div>
        </div>
    )
}